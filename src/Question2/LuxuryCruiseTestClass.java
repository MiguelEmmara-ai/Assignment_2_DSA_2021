...
public class LuxuryCruiseTestClass {
    public static void main(String[] args) {
        ...
        List<CruiseJourney> options = centre.getPossibleJourneys("Brisbane", new GregorianCalendar(2020, Calendar.JUNE, 8), "Antarctica");
        //options.sort(new JourneyComparator());
        System.out.println(">>>  There are " + options.size() + " possible journeys ");
        for (int i = 0; i < options.size(); i++) {
            System.out.println("-------------------OPTION " + (i + 1) + "--------------------------");
            for (int j = 0; j < options.size(); j++) {
                System.out.println("\nJourney Option " + (j + 1) + ": " + options.get(j));
            }
        }
    }
    ...
    private static class JourneyComparator implements Comparator<CruiseJourney> {

        public JourneyComparator() {
        }

        @Override
        public int compare(CruiseJourney arg0, CruiseJourney arg1) {
